package cc.landingzone.dreamcmp.demo.resourceapply;
...
public class ResourceApplyController extends BaseController {
    ...
    public void createEcsInstance(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            List<String> instanceDisplayList = ResourceApplyUtil.createEcsInstance(regionId, vpcId, vSwitchId, instanceType, amount, applicationName,
                    environmentName);
            result.setData(instanceDisplayList);
        } catch (Exception e) {
            logger.error(e.getMessage());
            result.setSuccess(false);
            result.setErrorMsg(e.getMessage());
